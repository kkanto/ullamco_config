...
def job_config = [
        job: [
                name: "ullamco_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "feature-1669638524980"
        ]
]
...
def customLib = library identifier: 'ullamco_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/ullamco_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.ullamco.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "ullamco", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("ullamco_Pipeline_develop");

job.with {

    authenticationToken('fnR7nv6Wx33d3LlqYgLSxPIesKs4Q0wN')
